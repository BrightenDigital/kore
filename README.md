<div align="center">

# 🧭 Compass Agentic Platform

### *Where Autonomous AI Agents Transform Enterprise Workflows*

[![Documentation](https://img.shields.io/badge/docs-latest-blue.svg)](https://brightendigital.github.io/compass/)
[![Platform](https://img.shields.io/badge/platform-Enterprise-purple.svg)](https://app.compassap.ai)
[![License](https://img.shields.io/badge/license-Enterprise-green.svg)]()
[![Status](https://img.shields.io/badge/status-Production-success.svg)]()

[**Try Compass**](https://app.compassap.ai) • [**Documentation**](https://brightendigital.github.io/compass/) • [**What's New**](https://brightendigital.github.io/compass/getting_started/whats_new.html)

</div>

---

## 🌟 Overview

**Compass** is a next-generation enterprise agentic platform that powers true AI autonomy through specialized agents working collaboratively to tackle complex business challenges. Built on a multi-agent, multi-action framework, Compass seamlessly integrates perception-to-action workflows, empowering your teams to focus on strategic decisions while AI agents handle the operational heavy lifting.

Unlike traditional AI assistants, Compass agents **observe, think, plan, and act autonomously**—transforming how enterprises handle software development, knowledge management, content generation, and complex workflow automation.

> ✨ **This documentation was entirely built using Compass AI**

---

## 🚀 Key Capabilities

### 🤖 **Autonomous AI Agent Teams**

Deploy specialized AI agents that work as cohesive teams, each bringing domain expertise:

- **🏗️ AI Software Team** - Full-stack development with Martha (Product Manager), Bruce (Architect), and Nova (Engineer)
- **📚 AI Knowledge Base** - Context-aware search and insights powered by Albert (Knowledge Seeker)
- **📊 SlideCraft Team** - Transform complex data into compelling PowerPoint and PDF presentations
- **🎯 Custom Enterprise Chatbots** - Build domain-specific AI assistants tailored to your organization

### 🧠 **Agentic RAG (Retrieval-Augmented Generation)**

Supercharge your agents with enterprise knowledge:

- **Dynamic Search Strategies** - Semantic, keyword, and fuzzy matching based on query context
- **Source Prioritization Engine** - Intelligent ranking based on relevance, freshness, and interaction patterns
- **Real-Time Knowledge Grounding** - Agents make decisions backed by your enterprise data
- **Bucket-Based Knowledge Management** - Organize and secure information with granular access control

### 🏢 **Enterprise-Grade Infrastructure**

Built for scale, security, and seamless integration:

| Feature | Description |
|---------|-------------|
| **🔗 Git Integration** | Native version control with push/pull, branch management, and seamless repository workflows |
| **☁️ Microsoft 365** | Direct integration with Graph API, Teams, OneDrive, and SharePoint |
| **🔌 Model Context Protocol (MCP)** | Connect custom tools and extend agent capabilities |
| **🛡️ Sensitive Data Masking** | Asynchronous PII processing with enhanced traceability and compliance |
| **🎛️ Custom LLM Configuration** | Bring your own models with tenant-specific settings |
| **👁️ Observability** | Complete audit trails and performance monitoring |
| **👥 Multi-Tenancy** | Secure organizational isolation with granular role-based access control |

### 📦 **Stack Templates & CompassRules**

Ensure consistency and best practices:

- **Predefined Templates** - Flutter, VitePress, SAP UI5, and more
- **Custom Templates** - Upload your own organizational standards
- **CompassRules** - Embedded best practices and guidelines per agent
- **Boilerplate Generation** - Accelerate project initialization with intelligent scaffolding

---

## 🎯 Use Cases

Compass excels across diverse enterprise scenarios:

- **🚀 Greenfield Development** - Rapid prototyping and application scaffolding
- **🔧 Legacy Modernization** - Code refactoring, migration, and enhancement
- **📖 Knowledge Discovery** - Extract insights from documents, codebases, and data sources
- **✅ Compliance & Documentation** - Generate audit-ready documentation and compliance reports
- **💼 Process Automation** - Automate complex, multi-step business workflows
- **🎓 Onboarding & Training** - Create intelligent assistants for team enablement

---

## 🏗️ Architecture Highlights

```
┌─────────────────────────────────────────────────────┐
│                  Compass Platform                    │
├─────────────────────────────────────────────────────┤
│  🤖 Multi-Agent Framework                           │
│    ├─ Agent Orchestration Layer                     │
│    ├─ Context Management & Memory                   │
│    └─ Tool Execution Engine                         │
├─────────────────────────────────────────────────────┤
│  🧠 Agentic RAG System                              │
│    ├─ Vector & Semantic Search                      │
│    ├─ Knowledge Graph Integration                   │
│    └─ Source Prioritization Engine                  │
├─────────────────────────────────────────────────────┤
│  🔌 Integration Layer                               │
│    ├─ Git (GitHub, GitLab, Bitbucket)              │
│    ├─ Microsoft 365 (Graph API)                     │
│    ├─ Model Context Protocol (MCP)                  │
│    └─ Custom Connectors                             │
├─────────────────────────────────────────────────────┤
│  🛡️ Security & Governance                           │
│    ├─ PII Masking & Data Controls                   │
│    ├─ Role-Based Access Control (RBAC)             │
│    ├─ Audit Logs & Observability                    │
│    └─ Tenant Isolation                              │
├─────────────────────────────────────────────────────┤
│  🎛️ LLM Management                                  │
│    ├─ Multi-Model Support (Claude, GPT, Gemini)    │
│    ├─ Custom Model Configuration                    │
│    └─ Intelligent Model Selection                   │
└─────────────────────────────────────────────────────┘
```

---

## 🚀 Getting Started

### 1️⃣ **Access the Platform**

Sign up and get started with Compass in minutes:

👉 **[Sign up now at app.compassap.ai](https://app.compassap.ai/)**

> 🔔 We're currently processing a high volume of access requests. Our team is actively reviewing and granting access as quickly as possible.

### 2️⃣ **Choose Your Path**

- **🆕 Start a New Project** - Select from our tech stack templates and describe your vision
- **📂 Connect Existing Assets** - Integrate with Git repositories or upload files
- **🔍 Unlock Knowledge** - Connect documents, databases, and web sources

### 3️⃣ **Engage Your Agents**

- **For Development** - Direct tasks to the AI Software Team or specific agents (@Martha, @Bruce, @Nova)
- **For Insights** - Query the Knowledge Base agent (Albert) with natural language
- **For Presentations** - Use SlideCraft to transform data into visual narratives

### 4️⃣ **Review & Iterate**

- Review agent outputs and provide feedback
- Iterate with conversational guidance
- Seamlessly integrate results into your workflow

---

## 📚 Documentation

Comprehensive guides and references:

- **[📖 Full Documentation](https://brightendigital.github.io/compass/)** - Complete platform guide
- **[🎯 Core Capabilities](https://brightendigital.github.io/compass/getting_started/tldr_key_features.html)** - Feature deep-dive
- **[💡 Real-World Examples](https://brightendigital.github.io/compass/agents/use_cases.html)** - Use case scenarios
- **[🔧 Quickstart Guide](https://brightendigital.github.io/compass/getting_started/quickstart.html)** - Get up and running fast
- **[🤖 Agent Documentation](https://brightendigital.github.io/compass/agents/home.html)** - Learn about each agent
- **[🏢 Organization Management](https://brightendigital.github.io/compass/org/org.html)** - Enterprise configuration
- **[🌐 Ecosystem & Integrations](https://brightendigital.github.io/compass/ecosystem/ecosystem.html)** - Integration guides
- **[🚦 What's New](https://brightendigital.github.io/compass/getting_started/whats_new.html)** - Latest updates and releases

---

## 🔧 Working with Documentation (Developers)

This repository contains the official Compass AI documentation built with [VitePress](https://vitepress.dev/).

### Prerequisites

- Node.js 18+ and npm

### Local Development

```bash
# Clone the repository
git clone https://github.com/BrightenDigital/compass.git
cd compass

# Install dependencies
npm install

# Start the development server
npm run docs:dev
```

The documentation site will be available at `http://localhost:5173`

### Building for Production

```bash
# Build the documentation
npm run docs:build

# Preview the production build
npm run docs:preview
```

### Documentation Structure

```
kore-docs/
├── getting_started/     # Onboarding guides and quickstarts
├── agents/              # Agent documentation and best practices
├── org/                 # Organization and enterprise features
├── ecosystem/           # Integrations and technical architecture
├── tutorials/           # Step-by-step tutorials
└── .vitepress/          # VitePress configuration and theme
```

---

## 🌐 Ecosystem

Compass is built on a powerful ecosystem of integrated technologies:

- **[Compass Agentic RAG](https://brightendigital.github.io/compass/ecosystem/compass_rag.html)** - Advanced retrieval and knowledge grounding
- **[Compass SLM](https://brightendigital.github.io/compass/ecosystem/compass_slm.html)** - Small language models for efficient inference
- **[Compass M365](https://brightendigital.github.io/compass/ecosystem/compass_m365.html)** - Microsoft 365 deep integration
- **[Compass MS Teams](https://brightendigital.github.io/compass/ecosystem/compass_msteams.html)** - Collaborative agent experiences
- **[System Blueprint](https://brightendigital.github.io/compass/technical/system_blueprint.html)** - Technical architecture overview

---

## 🗺️ Roadmap

We're continuously evolving Compass with exciting features on the horizon:

- 🔮 **Enhanced Multi-Modal Agents** - Vision, audio, and video understanding
- 🌍 **Global Marketplace** - Share and discover enterprise chatbots
- 🔐 **Advanced Compliance Frameworks** - SOC 2, GDPR, HIPAA presets
- 🧪 **Testing & Quality Agents** - Automated QA and test generation
- 📊 **Analytics Dashboard** - Deep insights into agent performance
- 🎨 **No-Code Agent Builder** - Visual agent configuration

See our [full roadmap](https://brightendigital.github.io/compass/ecosystem/roadmap.html) for details.

---

## 🤝 Contributing

We welcome contributions from the community! Whether you're fixing documentation typos, improving examples, or suggesting new features:

1. Fork the repository
2. Create a feature branch (`git checkout -b feature/amazing-feature`)
3. Commit your changes (`git commit -m 'Add amazing feature'`)
4. Push to the branch (`git push origin feature/amazing-feature`)
5. Open a Pull Request

---

## 📞 Support & Community

- **🐛 Report Issues** - [GitHub Issues](https://github.com/BrightenDigital/compass/issues)
- **💬 Discussions** - [GitHub Discussions](https://github.com/BrightenDigital/compass/discussions)
- **📧 Enterprise Support** - Contact your account manager
- **📖 FAQ** - [Frequently Asked Questions](https://brightendigital.github.io/compass/getting_started/frequently_asked.html)

---

## 📄 License

Compass is an enterprise platform. For licensing information, please contact our sales team.

---

## 🙏 Acknowledgments

Built with cutting-edge technologies:

- [Compass Agentic Platform](https://compassap.ai/) - Compass Agentic Platform

---

<div align="center">

**[🚀 Get Started with Compass Today](https://app.compassap.ai)**

Made with ❤️ by the Compass Team

Copyright © 2025 Compass Agentic Platform

</div>
